...
with engine.connect() as connection:
    connection.execute(sqlalchemy.text(
		...
		"Date" timestamp NULL,
		"Open" NUMERIC(9,2) NULL,
		"High" NUMERIC(9,2) NULL,
		"Low" NUMERIC(9,2) NULL,
		"Close" NUMERIC(9,2) NULL,
		"Volume" float NULL,
		symbol text NULL
		);
...
def Get_Data_From_Yahoo_full_load(stocklist,period='1y'):
	...
	for i in range(0,len(stocklist),100):
		...
		for idx,symbol in enumerate(downloadedsymbols):
			...
			main_big_df=pd.concat([main_big_df,indiv_df])
...
if not insp.has_table('stock_price_history',schema='public'):
	Get_Data_From_Yahoo_full_load(sp500symbols)

else:
	Get_Data_From_Yahoo_full_load(sp500symbols)
	# Get_Data_From_Yahoo_daily(sp500symbols) # needs fix
